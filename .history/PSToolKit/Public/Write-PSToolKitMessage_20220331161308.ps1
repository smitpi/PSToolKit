...
Function Write-PSToolKitMessage {
	...
	PARAM(
		...
		[Parameter(Mandatory = $true, $false, Position = 1)]
		[string]$Action,
		[ValidateSet('Information', 'Warning', 'Error')]
		[string]$Severity,
		[Parameter(Mandatory = $true, ValueFromPipeline = $true, ValueFromRemainingArguments = $false, Position = 0)]
		[string[]]$Object,
		[Parameter(Mandatory = $true, ValueFromPipeline = $true, ValueFromRemainingArguments = $false, Position = 0)]
		[string[]]$Message
	)
	...
	else {Write-Color "[$($Severity)]", "[$($Action)]", " $Object ", "$Message" -Color DarkCyan, Yellow, Cyan, Green }
