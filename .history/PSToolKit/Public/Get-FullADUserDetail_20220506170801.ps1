...
Function Get-FullADUserDetail {
	...
	if ($DomainCredential) {
		...
		$UserSummery = Get-ADUser $UserToQuery -Properties * -serv | Select-Object Name, GivenName, Surname, UserPrincipalName, EmailAddress, EmployeeID, EmployeeNumber, HomeDirectory, Enabled, Created, Modified, LastLogonDate, samaccountname
