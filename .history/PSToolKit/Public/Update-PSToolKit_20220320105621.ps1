...
Function Update-PSToolKit {
	...
	if ((Test-Path $ModulePath  )) {
		...
		if ($InstalledVer -lt $OnlineVer.Version) {
			...
			Write-Host '[Updateing]: ' -NoNewline -ForegroundColor Yellow; Write-Host "PSToolKit ($($OnlineVer.Version)): " -ForegroundColor Cyan -NoNewline; Write-Host "Already Installed" -ForegroundColor DarkRed
}
	} else {
