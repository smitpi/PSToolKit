...
Function Set-PSToolKitSystemSettings {
    ...
    PARAM(
        ...
        [switch]$InstallMSUpdates,
        [switch]$EnableNFSClient,
        [switch]$PerformReboot
    )

    if ($RunAll) {
        $ExecutionPolicy = $PSGallery = $IntranetZone = $IntranetZoneIPRange = $PSTrustedHosts = $FileExplorerSettings = $DisableIPV6 = $DisableFirewall = $DisableInternetExplorerESC = $DisableServerManager = $EnableRDP = $InstallPS7 = $InstallVMWareTools = $InstallAnsibleRemote = $InstallMSUpdates = $EnableNFSClient = $PerformReboot = $true
    }

    if ($RunPopular) {
        $RunAll = $ExecutionPolicy = $PSGallery = $IntranetZone = $IntranetZoneIPRange = $PSTrustedHosts = $FileExplorerSettings = $DisableIPV6 = $DisableFirewall = $DisableInternetExplorerESC = $DisableServerManager = $EnableRDP = $InstallPS7 = $InstallVMWareTools = $InstallAnsibleRemote = $InstallMSUpdates = $EnableNFSClient = $PerformReboot
    }
