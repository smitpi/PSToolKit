...
Function New-PSProfile {
    ...
    foreach ($folder in $folders) {
        if (-not(Test-Path ([IO.Path]::Combine($folder.FullName, 'Config')))) { $ConfigPath =  New-Item ([IO.Path]::Combine($folder.FullName, 'Config')) -ItemType Directory }
            ...
            $BCKDestination = ([IO.Path]::Combine($ConfigPath.FullName, $(Get-Date -Format yyyy.MM.dd_HH.mm)))
    }
        ...
        if (Test-Path ([IO.Path]::Combine($folder.FullName, $ise))) {
            ...
            Move-Item ([IO.Path]::Combine($folder.FullName, $ise)) -Destination ([IO.Path]::Combine($BCKDest.FullName,"ISEProfile-$(Get-Date -Format yyyy-MM-dd).ps1")) -Force }
