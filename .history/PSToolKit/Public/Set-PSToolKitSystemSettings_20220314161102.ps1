...
Function Set-PSToolKitSystemSettings {
        ...
        try {
            if ((Test-Path 'C:\Program Files\PowerShell\7') -eq $false) {
                ...
                if ($null -like $ReleaseModule) {
                    Write-Color '[Installing] ', 'Required Modules ' -Color Yellow, Cyan
    ...
    if ($InstallMSUpdates) {
                $ReleaseModule = Get-Module PSWindowsUpdate
                if ($null -like $ReleaseModule) {$ReleaseModule = Get-Module PSWindowsUpdate -ListAvailable}
                if ($null -like $ReleaseModule) {
                    Write-Color '[Installing] ', 'Required Modules: ', 'PSWindowsUpdate' -Color Yellow, Cyan
