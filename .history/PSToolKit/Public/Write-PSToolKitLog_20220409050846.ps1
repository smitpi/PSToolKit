...
Function Write-PSToolKitLog {
    ...
    if ($ExportFinal) { 
        ...
        if ($Export -eq 'HTML') { $ExportLogs | Out-HtmlView -DisablePaging -Title 'PrivRepoLog' -HideFooter -SearchHighlight -FixedHeader 
            Out-GridHtml -DisablePaging -Title "$($LogName)" -HideFooter -SearchHighlight -FixedHeader -FilePath $(Join-Path -Path $ReportPath -ChildPath "\$($LogName)-$(Get-Date -Format yyyy.MM.dd-HH.mm).html")
    }
