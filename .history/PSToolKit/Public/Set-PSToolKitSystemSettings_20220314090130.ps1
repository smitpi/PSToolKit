...
Function Set-PSToolKitSystemSettings {
        ...
        foreach ($ip in $IPAddresses) {
            ...
            try {
                ...
                if (-not(Test-Path -Path $keychild)) {
                    ...
                    Write-Color '[Set]', "IntranetZone IP Range:$($index)", ' Already Exits' -Color Yellow, Cyan, DarkRed
                    ++$index
                }
    ...
    if ($DisableFirewall) {
        try {
            ...
            else {Write-Color '[Disable]', 'Firewall: ', 'Already Set' -Color Yellow, Cyan, Green}
    ...
    if ($EnableRDP) {
        try {
            ...
            else {Write-Color '[Enable]', 'RDP: ', 'Already Set' -Color Yellow, Cyan, Green}
    ...
    if ($InstallVMWareTools) {
        try {
            ...
            else {Write-Color "[Installing]","VMWare Tools:"," Not a VMWare VM" -Color Yellow,Cyan,green}
