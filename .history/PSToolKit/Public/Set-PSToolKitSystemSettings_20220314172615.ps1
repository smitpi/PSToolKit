...
Function Set-PSToolKitSystemSettings {
    ...
    if ($InstallRSAT) {
         ...
         Install-ChocolateyClient
        Write-Color '[Installing] ', 'RSAT Tools', ' from source ', 'chocolatey' -Color Yellow, Cyan, green, Cyan
        ...
        Write-Color '[Installing] ', 'RSAT Tools: ', 'Domain Name Service' -Color Yellow, Cyan, green
        choco upgrade rsat -params '"/AD"' --accept-license --limit-output -y --source chocolatey | Out-Null
        if ($LASTEXITCODE -ne 0) {Write-Warning "Error Installing vmware-tools Code: $($LASTEXITCODE)"}
        ...
        choco upgrade rsat -params '"/AD /GP /SM /FS /DNS /DHCP"'
    }
