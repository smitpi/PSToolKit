    ...
    if ($InstallRSAT) {
        try {
        	...
        	if ($checkver -notlike '*server*') {
                Write-Color '[Installing] ', 'RSAT Tools', ' for a ', 'Workstation' -Color Yellow, Cyan, green, Cyan
                ...
                $Roles | ForEach-Object {
                    ...
                    else {Write-Color '[Installing] ', 'RSAT Tool: ', "$($_.split('~')[0].split('.')[1])-$($_.split('~')[0].split('.')[2])","Already Installed" -Color Yellow, Cyan, green, Cyan,DarkRed
