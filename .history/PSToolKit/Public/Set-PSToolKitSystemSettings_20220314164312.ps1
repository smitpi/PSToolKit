...
Function Set-PSToolKitSystemSettings {
    ...
    if ($RunPopular) {
        $RunAll = $ExecutionPolicy = $PSGallery = $IntranetZone = $IntranetZoneIPRange = $PSTrustedHosts = $FileExplorerSettings = $DisableIPV6 = $DisableFirewall = $DisableInternetExplorerESC = $DisableServerManager = $EnableRDP = $InstallPS7 = $InstallVMWareTools = $InstallAnsibleRemote
    }
