...
Function Install-PSModules {
	...
	if ($UpdateModules) {
		try {
			$mods | ForEach-Object {
				Write-Color '[Installing] ', $($_.Name), 'to Scope: ', $($Scope) -Color Yellow, Cyan, Green, Cyan
				 Write-Host 'Updating Module:' -ForegroundColor Cyan -NoNewline; Write-Host $_.Name -ForegroundColor yello
	...
	foreach ($mod in $mods) {
		...
		if ($PSModule.Name -like '') {
			...
			Write-Color '[Installing] ', $($PSModule.Name),"Path: ", $($PSModule.Path), "Already Installed" -Color Yellow, Cyan, Green, Cyan, DarkRed
		}
	}
}
