...
Function Start-PSToolkitSystemInitialize {
	...
	if ($LabSetup) {
		New-PSProfile
		Set-PSToolKitSystemSettings -ExecutionPolicy -PSGallery -IntranetZone -IntranetZoneIPRange 
		Set-PSToolKitSystemSettings -PSTrustedHosts -FileExplorerSettings -DisableIPV6 -DisableFirewall -DisableInternetExplorerESC -DisableServerManager -EnableRDP 
		Set-PSToolKitSystemSettings -InstallVMWareTools -InstallAnsibleRemote -EnableNFSClient
