...
Function $properverb-$propernoun {
	...
	if (`$Export -eq 'Excel') { `$data | Export-Excel -Path `$(Join-Path -Path `$ReportPath -ChildPath "\$propernoun-`$(Get-Date -Format yyyy.MM.dd-HH.mm).xlsx") -AutoSize -AutoFilter -Show 
		$data | Export-Excel -Path $(Join-Path -Path $ReportPath -ChildPath "\$propernoun-$(Get-Date -Format yyyy.MM.dd-HH.mm).xlsx") -WorksheetName $propernoun -AutoSize -AutoFilter -Title 'Lic Details' -TitleBold -TitleSize 28
	}
