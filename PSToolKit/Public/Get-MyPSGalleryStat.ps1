...
Function Get-MyPSGalleryStat {
    ...
    if (-not([string]::IsNullOrEmpty($GitHubUserID)) -and -not([string]::IsNullOrEmpty($GitHubToken))) {
        ...
        if ($History) {
            ...
            $GalHistory | Where-Object {$_.date -gt $end}
            $allNames = $GalHistory.details.Sum | Where-Object {$_.date -lt $end} | Select-Object Name | Sort-Object -Property Name -Unique

            foreach ($All in $allNames) {
                $sum = $GalHistory.details.Sum | Where-Object {$_.Name -like $all.Name -and $_.date -lt $end}
