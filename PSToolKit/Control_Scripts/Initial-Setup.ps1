...
if ($EnableWSL -and ($WSLUser -notlike 'None')) {
	if (-not(Test-Path "$($PSDownload.fullname)\WSL.tmp")) {
		...
		Run-Block -Name Install_WSL -Block "choco install wsl2 --yes --params `"/Version:2 /Retry:true`""
        check-reboot 
        Run-Block -Name Install_Ubuntu -Block "choco install wsl-ubuntu-2204 --yes --ignore-checksums --params `"/AutomaticInstall:true`""
        check-reboot
        #Invoke-Command -ScriptBlock { 
