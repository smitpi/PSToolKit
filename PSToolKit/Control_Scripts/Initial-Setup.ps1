...
if ($EnableWSL -and ($WSLUser -notlike 'None')) {
	if (-not(Test-Path "$($PSDownload.fullname)\WSL.tmp")) {
		...
		$LinuxUserSetup = {
			...
			cmd.exe /c 'wsl --distribution Ubuntu --user root --exec cat /etc/wsl.conf'
			cmd.exe /c 'wsl --shutdown Ubuntu'
		}

		$DeployAnsible = {
			cmd.exe /c 'wsl --distribution Ubuntu --user root --exec sudo apt update'
			cmd.exe /c 'wsl --distribution Ubuntu --user root --exec sudo apt install make git -y'
			cmd.exe /c "wsl --distribution Ubuntu --user root --exec sudo git clone https://$($GitHubToken):x-oauth-basic@github.com/smitpi/ansible-bootstrap /home/$($WSLUser)/ansible/ansible-bootstrap"
			cmd.exe /c "wsl --distribution Ubuntu --user root --exec sudo cp /home/$($WSLUser)/ansible/ansible-bootstrap/inventory-src /home/$($WSLUser)/ansible/inventory"
			cmd.exe /c "wsl --distribution Ubuntu --user root --exec sudo mkdir /home/$($WSLUser)/ansible/host_vars"

			cmd.exe /c 'wsl --distribution Ubuntu --user root --exec sudo apt install git python3-pip python3-dev -y'
			cmd.exe /c 'wsl --distribution Ubuntu --user root --exec sudo pip3 install ansible'
		}
